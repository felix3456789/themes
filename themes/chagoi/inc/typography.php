...
if ( ! function_exists( 'chagoi_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'chagoi_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function chagoi_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', chagoi_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $chagoi_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $chagoi_settings[$key], ':' ) !== false ) {
					$chagoi_settings[$key] = current( explode( ':', $chagoi_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $chagoi_settings[$key] );

				// Grab the variants using the plain name
				$variants = chagoi_get_google_font_variants( $chagoi_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'chagoi_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'chagoi_fonts_subset','' );
		...
		$default_fonts = '//fonts.googleapis.com/css?family=Open+Sans+Condensed:300,300italic,700|Dosis:200,300,regular,500,600,700,800|Abril Fatface:regular';

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('chagoi-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('chagoi-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'chagoi_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'chagoi_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function chagoi_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'chagoi_fonts_customize_register' ) ) {
			// Bail if Chagoi Premium is active
			return;
		}
		...
		$defaults = chagoi_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Chagoi_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'chagoi_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'chagoi_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'chagoi_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Chagoi_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'chagoi' ),
					'settings' => array(
						'family' => 'chagoi_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'chagoi_settings[body_font_weight]',
						'transform' => 'chagoi_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_setting(
			'chagoi_settings[body_font_size]',
			array(
		...
		$wp_customize->add_control(
			new Chagoi_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'chagoi' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'chagoi_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'chagoi_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'chagoi_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Chagoi_Range_Slider_Control(
				...
				array(
					'type' => 'chagoi-range-slider',
					'description' => __( 'Line height', 'chagoi' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'chagoi_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'chagoi_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Chagoi_Range_Slider_Control(
				...
				array(
					'type' => 'chagoi-range-slider',
					'description' => __( 'Paragraph margin', 'chagoi' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'chagoi_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'chagoi_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function chagoi_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'chagoi_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'chagoi_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'chagoi_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_chagoi_get_all_google_fonts_ajax', 'chagoi_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function chagoi_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'chagoi_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'chagoi_customize_nonce' ] ), 'chagoi_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'chagoi_customize_nonce', 'chagoi_customize_nonce' );
		...
		$fonts = chagoi_get_all_google_fonts();
...
if ( ! function_exists( 'chagoi_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: chagoi_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function chagoi_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, chagoi_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'chagoi_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'chagoi_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: chagoi_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function chagoi_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'chagoi_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function chagoi_get_font_family_css( $font, $settings, $default ) {
		$chagoi_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'chagoi_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $chagoi_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'chagoi_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . chagoi_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'chagoi_add_to_font_customizer_list' ) ) {
	add_filter( 'chagoi_typography_customize_list', 'chagoi_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function chagoi_add_to_font_customizer_list( $fonts ) {
		...
		$all_fonts = chagoi_get_all_google_fonts();
		$select_fonts = chagoi_get_all_google_fonts( apply_filters( 'chagoi_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $chagoi_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $chagoi_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $chagoi_settings[ $setting ], chagoi_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $chagoi_settings[ $setting ] ) ) ] = array(
				'name' => $chagoi_settings[ $setting ],
		...
		if ( function_exists( 'chagoi_secondary_nav_get_defaults' ) ) {
			$secondary_nav_settings = wp_parse_args(
				get_option( 'chagoi_secondary_nav_settings', array() ),
				chagoi_secondary_nav_get_defaults()
			);

			$secondary_nav_id = strtolower( str_replace( ' ', '_', $secondary_nav_settings[ 'font_secondary_navigation' ] ) );

			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], chagoi_typography_default_fonts() ) ) {
