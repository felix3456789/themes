...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Chagoi_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Chagoi_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'chagoi-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'chagoi-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), CHAGOI_VERSION, true );
			wp_enqueue_style( 'chagoi-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), CHAGOI_VERSION );

			wp_enqueue_script( 'chagoi-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'chagoi-typography-selectWoo' ), CHAGOI_VERSION, true );
			wp_enqueue_style( 'chagoi-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), CHAGOI_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'chagoi_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'chagoi' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'chagoi' );
			$this->json[ 'google_fonts' ] = apply_filters( 'chagoi_typography_customize_list', chagoi_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = chagoi_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'chagoi' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'chagoi' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'chagoi' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'chagoi' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="chagoi-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="chagoi-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="chagoi-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="chagoi-font-transform">
					<label>
